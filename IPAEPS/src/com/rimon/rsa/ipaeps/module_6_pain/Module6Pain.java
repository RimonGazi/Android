...
public class Module6Pain extends SherlockFragmentActivity implements
		AnyPageChangeListener, AnyDialogPasswordListener {
	private AnySwipeableViewPager pager;
	private PainSectionsPagerAdapter adapter;
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.module_6_pain_main);
		tv_action_bar = (AnyTextView) findViewById(R.id.tv_action_bar);
		tv_action_bar.setText(this.getResources().getString(
				R.string.pain_symptoms));
		im_action_bar = (ImageView) findViewById(R.id.im_action_bar);
		im_action_bar.setBackgroundResource(R.drawable.ic_actionber_pain);
		pager = (AnySwipeableViewPager) findViewById(R.id.pain_pager);

		pager.setSwipeable(false);

		adapter = new PainSectionsPagerAdapter(getSupportFragmentManager());
		try {
